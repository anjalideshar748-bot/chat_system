            ...
            <div class="flex-1 overflow-y-auto space-y-1 px-2">
                @foreach ($user as $user)
                    <div class=" flex row-auto gap-4 px-3 py-3 rounded-lg bg-[#3c8c6d] cursor-pointer">
                        ...
                        @if ($me->friends()->where('friend_id', $user->id)->where('status', 'accepted')->exists())
                            <span class="text-sm bg-green-500 text-white px-2 py-1 rounded">Friends</span>
                        @elseif ($me->friends()->where('friend_id', $user->id)->where('status', 'pending')->exists())
                            <span class="text-sm bg-yellow-500 text-white px-2 py-1 rounded">Request Sent</span>
                        @else
                         <form method="POST"
